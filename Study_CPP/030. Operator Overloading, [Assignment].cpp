...
- 단, 전역으로 정의해줄 때에는 인자 중 하나는 무조건 객체 타입이어야 합니다.
...
- 컴파일러가 생성 해준 "Default Copy Assignment Operator"는
멤버들 간 단순 대입을 해주기 때문에 "얕은 복사"를 진행합니다.
(메모리 동적 할당한 멤버를 갖는 경우, 대입 연산자도 깊은 복사 방식으로 오버로딩하여 구현해 준다 !!!!)



복사 대입 연산자의 Operator Overloading - Default 형태, 멤버 간 단순 대입.
Point& operator= (Point &ref)
{
...
(이미 만들어진 객체를 대상으로 대입 연산을 해줄 때, 복사 대입 연산자가 호출됩니다 !!!!
########################################################################################
...
public:
	...
	Point operator+ (int n) // 해당 클래스 객체 대상으로 int 자료형이 덧셈 될 때 !!!!
	{
...
int main(void)
{
	...
	cout << endl;				   //
								   ...
								   // 단, 전역으로 정의해줄 때에는 인자 중 하나는 무조건 객체 타입이어야 한다.
	...
	cout << endl;							  // 함수 구현에서 "Point operator+ (int n);"을 사용함.
											  // 지역 변수에 저장하는 것이 아니고 내부에서 임시 객체를 생성하고 바로 return 하기 때문에,
											  // (임시 객체 생성 시, 복사 생성자 호출 1번하는 것으로 컴파일러가 최적화를 해주었다 !!!!)
											  // #######################################################################################
